...
# from .ssconv_operation import ScConv1
from .my_ssconv_operation import ScConv
from .paper import SFCM
...
class RepVggBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # hasattr 是 Python 内置函数，用于检查对象是否有指定的属性或方法。hasattr(object, name) 接受两个参数：
        # object: 要检查的对象。
        # name: 属性或方法的名称。
...
class HybridEncoder(nn.Module):
    def __init__(self,
        ...
        # 自己改的
        # self.ScConv = ScConv(hidden_dim)
        self.SFCM = SFCM(hidden_dim)
    ...
    def forward(self, feats):
        ...
        # 自己加的 ScConv
        # proj_feats[self.use_encoder_idx[0]] = self.ScConv(proj_feats[self.use_encoder_idx[0]])
        proj_feats[self.use_encoder_idx[0]] = self.SFCM(proj_feats[self.use_encoder_idx[0]])
        # 论文中使用了1层
        # encoder
